...
import { BitacoraSupervisor } from "@/hooks/useBitacorasSupervisor";

interface BitacorasSupervisorManagerProps {
  bitacoras: BitacoraSupervisor[];
}

export const BitacorasSupervisorManager = ({ bitacoras }: BitacorasSupervisorManagerProps) => {
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{bitacora.supervisor}</p>
                    </div>
                  </div>
              ...
              <CardContent>
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="outline">{bitacora.evaluacion_general}</Badge>
                  </div>

                  {bitacora.desarrollo_inmersion && (
                    <div className="mt-3">
                      <p className="text-sm font-medium text-gray-700 mb-1">Desarrollo:</p>
                      <p className="text-xs text-gray-600 line-clamp-2">{bitacora.desarrollo_inmersion}</p>
                    </div>
                  )}

                  {bitacora.incidentes && (
                    <div className="mt-3">
                      <p className="text-sm font-medium text-gray-700 mb-1">Incidentes:</p>
                      <p className="text-xs text-gray-600 line-clamp-2">{bitacora.incidentes}</p>
                    </div>
                  )}
      ...
      {bitacoras.length === 0 && (
        <Card className="text-center py-12">
          <CardContent>
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No hay bitácoras de supervisor</h3>
            <p className="text-gray-500">Comience creando la primera bitácora de supervisor</p>
          </CardContent>
        </Card>
      )}
    </div>
