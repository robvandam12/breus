...
export const AnexoBravoDataTable = () => {
  ...
  const { data: anexoDocuments = [], isLoading } = useQuery({
    queryKey: ['anexo-bravo-documents'],
    queryFn: async () => {
  ...
  const filteredDocuments = anexoDocuments.filter(doc => {
      ...
      <Card>
        <CardContent className="p-6">
          <div className="text-center">
            ...
            <p className="mt-2 text-gray-600">Cargando documentos Anexo Bravo...</p>
          </div>
        </CardContent>
      </Card>
    );
  ...
  return (
    <>
          ...
          {filteredDocuments.length === 0 ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-lg font-medium text-gray-900 mb-2">
                No hay documentos Anexo Bravo
              </h3>
              <p className="text-gray-600">
                {searchTerm || statusFilter !== "all" 
                  ? "No se encontraron documentos con los filtros aplicados." 
                  : "Comienza creando tu primer documento Anexo Bravo."}
              </p>
            </div>
          ) : (
      ...
      {/* Modal para ver Anexo Bravo */}
      {viewingAnexo && (
        <Dialog open={!!viewingAnexo} onOpenChange={() => setViewingAnexo(null)}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Ver Anexo Bravo - {viewingAnexo.codigo}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-sm font-medium text-gray-700">Código</label>
                  <p className="font-mono">{viewingAnexo.codigo}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Estado</label>
                  <Badge className={getStatusColor(viewingAnexo.estado)}>
                    {viewingAnexo.estado}
                  </Badge>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Supervisor</label>
                  <p>{viewingAnexo.supervisor || 'No asignado'}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-gray-700">Fecha</label>
                  <p>{viewingAnexo.fecha ? new Date(viewingAnexo.fecha).toLocaleDateString('es-CL') : 'No definida'}</p>
                </div>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Lugar de Faena</label>
                <p className="text-sm text-gray-600 mt-1">
                  {viewingAnexo.lugar_faena || 'No especificado'}
                </p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-700">Empresa</label>
                <p className="text-sm text-gray-600 mt-1">
                  {viewingAnexo.empresa_nombre || 'No especificada'}
                </p>
              </div>
            </div>
          </DialogContent>
