          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {sitios.reduce((acc, s) => acc + (s.capacidad_jaulas || 0), 0)}
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">
                  {sitios.length > 0 
                    ? Math.round(sitios.reduce((acc, s) => acc + (s.profundidad_maxima || 0), 0) / sitios.length)
                    : 0
                  }m
                </p>
              </div>
        ...
        <CardContent>
                ...
                <TableRow key={sitio.id}>
                  <TableCell>
                    <div>
                      ...
                      <div className="text-sm text-gray-500">{sitio.region}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="font-mono text-sm">{sitio.codigo}</div>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">{sitio.ubicacion}</div>
                  </TableCell>
                  <TableCell>
                    <Badge variant="outline">{sitio.capacidad_jaulas || 0} jaulas</Badge>
                  </TableCell>
                  <TableCell>
                    <div className="text-sm">{sitio.profundidad_maxima || 0}m</div>
                  </TableCell>
                  <TableCell>
                    <Badge 
                      variant="outline" 
          ...
          {filteredSitios.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              ...
              <p>No se encontraron sitios que coincidan con la búsqueda.</p>
            </div>
          )}
        </CardContent>
      </Card>
