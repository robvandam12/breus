...
export const UserSearchSelectEnhanced = ({
  ...
  const { usuarios: salmoneraUsers, inviteUser } = useUsersByCompany(salmoneraId, 'salmonera');
  ...
  const handleInviteSubmit = (e: React.FormEvent) => {
    ...
    onInviteUser(inviteDataWithCompany);
  ...
  return (
    <div className="space-y-4">
                          ...
                          {allowedRoles.length > 0 ? (
                            allowedRoles.map(role => (
                              <option key={role} value={role}>
                                {role === 'supervisor' ? 'Supervisor' : 'Buzo'}
                              </option>
                            ))
                          ) : (
                            <>
      ...
      {/* Botón separado para invitar cuando no hay búsqueda */}
      {!searchTerm && (
